# Master Prompt: Synaptic-Impulse

## Objective
Replicate the **Synaptic-Impulse** cursor effect with 100% fidelity. This is a premium, high-performance cursor strictly for desktop web applications.
...
<body class="h-screen flex flex-col bg-[#050505] overflow-hidden cursor-none">
    ...
    <div style="position:fixed; bottom:20px; left:20px; z-index:100; font-family:'Rajdhani', sans-serif; pointer-events:none;">
        <h1 class="text-3xl font-bold text-white tracking-widest mb-1">SYNAPTIC-IMPULSE</h1>
...
// --- 1. SHARED PARTICLE POOL ---
      ...
      PROTOCOLS.push({
        ...
        color: "#ec4899",
        physics: { lag: 8, friction: 0.92 },
        theme: {
          tech: "Bio-Electric Transmission",
          philosophy: "Electricity through living tissue.",
          context: "Neural Interface.",
          emotion: "Alive",
        },
        cursor: {
          dims: "18px",
          shape: "Soma (Cell Body)",
          colors: "#EC4899 / #F9A8D4",
        },
        trail: {
          type: "Dendrite Fiber",
          mechanics: "Jagged History + Impulse Signals",
          visuals: "Nerve Glow",
        },
        config: { color: "#ec4899", physics: { lag: 8 } },

        triggerClick: function (renderer) {
          for (let i = 0; i < 3; i++) {
            renderer.pulses.push({
              index: renderer.history.length - 1,
        ...
        render: function (dt, renderer) {
          ...
          const cx = renderer.cursor.x,
            cy = renderer.cursor.y;

          renderer.history.push({ x: cx, y: cy, life: 1.0 });
          if (renderer.history.length > 40) renderer.history.shift();
          ...
          if (renderer.history.length > 2) {
            ctx.beginPath();
            for (let i = 0; i < renderer.history.length; i++) {
              const h = renderer.history[i];
              h.life -= dt * 0.5;
              const jitterX = (Math.random() - 0.5) * 3;
              const jitterY = (Math.random() - 0.5) * 3;
              if (i === 0) ctx.moveTo(h.x + jitterX, h.y + jitterY);
              else ctx.lineTo(h.x + jitterX, h.y + jitterY);
            }
            ctx.stroke();
          }

          if (renderer.speed > 5 && Math.random() > 0.95) {
            renderer.pulses.push({
              index: renderer.history.length - 1,
          ...
          for (let i = renderer.pulses.length - 1; i >= 0; i--) {
            const pulse = renderer.pulses[i];
            pulse.index -= pulse.speed;
            if (pulse.index < 0) {
              renderer.pulses.splice(i, 1);
              continue;
            }
            const historyIndex = Math.floor(pulse.index);
            if (historyIndex >= 0 && historyIndex < renderer.history.length) {
              const point = renderer.history[historyIndex];
              ctx.shadowBlur = 20;
          ...
          ctx.beginPath();
          const baseRadius = 12 + renderer.clickPulse * 5;
          for (let i = 0; i < Math.PI * 2; i += 0.3) {
            const noise =
              Math.sin(i * 5 + renderer.time * 10) * 2 +
              Math.sin(i * 3 - renderer.time * 7) * 1.5;
