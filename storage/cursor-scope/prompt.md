# Master Prompt: The Oscilloscope

## Objective
Replicate the **The Oscilloscope** cursor effect with 100% fidelity. This is a premium, high-performance cursor strictly for desktop web applications.
...
<body class="h-screen flex flex-col bg-[#050505] overflow-hidden cursor-none">
    ...
    <div style="position:fixed; bottom:20px; left:20px; z-index:100; font-family:'Rajdhani', sans-serif; pointer-events:none;">
        <h1 class="text-3xl font-bold text-white tracking-widest mb-1">THE OSCILLOSCOPE</h1>
...
// --- 1. SHARED PARTICLE POOL ---
      ...
      PROTOCOLS.push({
        ...
        color: "#39ff14",
        physics: { lag: 25, friction: 0.82 },
        theme: {
          tech: "Vector Graphics Terminal",
          philosophy: "Everything is a frequency.",
          context: "Tektronix 4010 Emulation.",
          emotion: "Electric",
        },
        cursor: {
          dims: "Variable",
          shape: "Lissajous Beam",
          colors: "Phosphor Green",
        },
        trail: {
          type: "Signal Decay",
          mechanics: "Analog Noise + Jitter",
          visuals: "CRT Bloom",
        },
        config: { color: "#39ff14", physics: { lag: 25 } },
        ...
        render: function (dt, renderer) {
          ...
          renderer.oscilloscopeTime += dt * 5;
          const t = renderer.oscilloscopeTime;
          ...
          if (renderer.history.length > 1) {
            ctx.beginPath();
            for (let i = 0; i < renderer.history.length - 1; i++) {
              ...
              ctx.lineTo(renderer.history[i + 1].x, renderer.history[i + 1].y);
            }
            ctx.stroke();
          }
          renderer.history = renderer.history.filter((h) => h.life > 0);
          ...
          ctx.beginPath();
          const size = 15 + renderer.speed * 0.1 + renderer.clickPulse * 10;
          for (let i = 0; i < Math.PI * 2; i += 0.05) {
            const freqX = 3 + renderer.speed * 0.01;
            const freqY = 2 + renderer.speed * 0.01;
            const x = Math.sin(i * freqX + t) * size;
            const y = Math.cos(i * freqY + t) * size;
            ctx.lineTo(x, y);
          }
          ctx.closePath();
          ctx.stroke();
