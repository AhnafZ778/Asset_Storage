# Master Prompt: Hard-Light Architect

## Objective
Replicate the **Hard-Light Architect** cursor effect with 100% fidelity. This is a premium, high-performance cursor strictly for desktop web applications.
...
<body class="h-screen flex flex-col bg-[#050505] overflow-hidden cursor-none">
    ...
    <div style="position:fixed; bottom:20px; left:20px; z-index:100; font-family:'Rajdhani', sans-serif; pointer-events:none;">
        <h1 class="text-3xl font-bold text-white tracking-widest mb-1">HARD-LIGHT ARCHITECT</h1>
...
// --- 1. SHARED PARTICLE POOL ---
      ...
      // 04 // HARD-LIGHT ARCHITECT
      PROTOCOLS.push({
        id: "architect",
        name: "HARD-LIGHT ARCHITECT",
        color: "#fbbf24",
        physics: { lag: 18, friction: 0.8 },
        theme: {
          tech: "Photonic Lattice",
          philosophy: "Order over chaos. Geometry over fluidity.",
          context: "Planetary Construction AI.",
          emotion: "Structure",
        },
        cursor: { dims: "28px", shape: "Precision Bracket", colors: "#FBBF24" },
        trail: {
          type: "Ortho-Grid",
          mechanics: "Manhattan Distance",
          visuals: "Vector Lines",
        },
        config: { color: "#fbbf24", physics: { lag: 18 } },
        ...
        render: function (dt, renderer) {
          ...
          const cx = Math.round(renderer.cursor.x),
            cy = Math.round(renderer.cursor.y);
          ...
          ctx.lineWidth = 1;
          for (let i = 0; i < renderer.history.length - 1; i += 2) {
            const p = renderer.history[i],
              next = renderer.history[i + 1];
            p.life -= 2.0 * dt;
            if (p.life <= 0) continue;
            ctx.globalAlpha = p.life * 0.8;
            ctx.beginPath();
            ctx.moveTo(p.x + 0.5, p.y + 0.5);
            ctx.lineTo(next.x + 0.5, p.y + 0.5);
          ...
          ctx.save();
          ctx.translate(cx, cy);
          const size = renderer.hoverState ? 20 : 16,
            gap = renderer.clickState ? 0 : 8;
          ctx.strokeStyle = "#fbbf24";
          ctx.lineWidth = 2;
          ctx.beginPath();
          ...
          ctx.lineTo(-size, size - gap);
          ctx.stroke();
          if (renderer.hoverState) ctx.rotate(Math.PI / 4);
          ctx.fillStyle = "#fff";
