# Master Prompt: Plasma Scout

## Objective
Replicate the **Plasma Scout** cursor effect with 100% fidelity. This is a premium, high-performance cursor strictly for desktop web applications.
...
<body class="h-screen flex flex-col bg-[#050505] overflow-hidden cursor-none">
    ...
    <div style="position:fixed; bottom:20px; left:20px; z-index:100; font-family:'Rajdhani', sans-serif; pointer-events:none;">
        <h1 class="text-3xl font-bold text-white tracking-widest mb-1">PLASMA SCOUT</h1>
...
// --- 1. SHARED PARTICLE POOL ---
      ...
      PROTOCOLS.push({
        ...
        name: "PLASMA SCOUT",
        color: "#ff4800",
        physics: { lag: 8, friction: 0.88 },
        theme: {
          tech: "Fusion Containment",
          philosophy: "Raw energy contained by magnetic fields.",
          context: "Orbital Mining.",
          emotion: "Power",
        },
        cursor: { dims: "32px", shape: "Reactor Core", colors: "#FF4800" },
        trail: {
          type: "Particle Jet",
          mechanics: "Object Pool Emitter",
          visuals: "Thermal Cooling",
        },
        config: { color: "#ff4800", physics: { lag: 8 } },
        ...
        render: function (dt, renderer) {
          ...
          if (renderer.speed > 5 || Math.random() > 0.5) {
            renderer.pool.spawn({
              x: renderer.cursor.x + (Math.random() - 0.5) * 10,
              y: renderer.cursor.y + (Math.random() - 0.5) * 10,
          ...
          ctx.rotate(-time * 2.5);
          const pulse = 1 + renderer.clickPulse * 0.5;
