# Master Prompt: Void Drifter

## Objective
Replicate the **Void Drifter** cursor effect with 100% fidelity. This is a premium, high-performance cursor strictly for desktop web applications.
...
<body class="h-screen flex flex-col bg-[#050505] overflow-hidden cursor-none">
    ...
    <div style="position:fixed; bottom:20px; left:20px; z-index:100; font-family:'Rajdhani', sans-serif; pointer-events:none;">
        <h1 class="text-3xl font-bold text-white tracking-widest mb-1">VOID DRIFTER</h1>
...
// --- 1. SHARED PARTICLE POOL ---
      ...
      PROTOCOLS.push({
        ...
        name: "VOID DRIFTER",
        color: "#8b5cf6",
        physics: { lag: 10, friction: 0.94 },
        theme: {
          tech: "Null-Point Drive",
          philosophy: "Movement without displacement.",
          context: "Stealth Drone Uplink.",
          emotion: "Mystery",
        },
        cursor: { dims: "20x34px", shape: "Rhombus Eye", colors: "#8B5CF6" },
        trail: {
          type: "Chromatic Echo",
          mechanics: "Distance Stamping",
          visuals: "Alpha Decay",
        },
        config: { color: "#8b5cf6", physics: { lag: 10 } },
        ...
        render: function (dt, renderer) {
          ...
          const lastPos =
            renderer.history.length > 0
              ? renderer.history[renderer.history.length - 1]
              : { x: 0, y: 0 };

          if (
            Math.hypot(
              renderer.cursor.x - lastPos.x,
              renderer.cursor.y - lastPos.y
            ) > 30
          ) {
            renderer.history.push({
              ...
              y: renderer.cursor.y,
              life: 1.0,
              angle: renderer.velocity.x * 0.05,
            });
            if (renderer.history.length > 10) renderer.history.shift();
          }
          ...
          renderer.history.forEach((ghost) => {
            ghost.life -= 1.5 * dt;
            if (ghost.life <= 0) return;
            ctx.save();
          ...
          ctx.rotate(renderer.velocity.x * 0.05);
