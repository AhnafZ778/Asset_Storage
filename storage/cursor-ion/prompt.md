# Master Prompt: Ion-Stream Interceptor

## Objective
Replicate the **Ion-Stream Interceptor** cursor effect with 100% fidelity. This is a premium, high-performance cursor strictly for desktop web applications.
...
<body class="h-screen flex flex-col bg-[#050505] overflow-hidden cursor-none">
    ...
    <div style="position:fixed; bottom:20px; left:20px; z-index:100; font-family:'Rajdhani', sans-serif; pointer-events:none;">
        <h1 class="text-3xl font-bold text-white tracking-widest mb-1">ION-STREAM INTERCEPTOR</h1>
...
// --- 1. SHARED PARTICLE POOL ---
      ...
      // 09 // ION-STREAM INTERCEPTOR
      PROTOCOLS.push({
        id: "ion",
        name: "ION-STREAM INTERCEPTOR",
        color: "#3b82f6",
        physics: { lag: 8, friction: 0.94 },
        theme: {
          tech: "Magnetoplasmadynamic Thruster",
          philosophy: "Thrust equals authority.",
          context: "Orbital Superiority Fighter.",
          emotion: "Focus",
        },
        cursor: {
          dims: "32px",
          shape: "Split-Wing Stealth",
          colors: "Titanium / Blue Flame",
        },
        trail: {
          type: "Volumetric Exhaust",
          mechanics: "Gaussian Particle Emitter",
          visuals: "Heat Gradient",
        },
        config: { color: "#3b82f6", physics: { lag: 8 } },
        ...
        render: function (dt, renderer) {
          ...
          const ax = Math.cos(renderer.angle);
          const ay = Math.sin(renderer.angle);
          const px = -ay;
          const py = ax;

          const spawnCount =
            renderer.speed > 2 ? 2 : Math.random() > 0.8 ? 1 : 0;

          for (let i = 0; i < spawnCount; i++) {
            const side = i % 2 === 0 ? 1 : -1;
            const spread = 8;
            renderer.pool.spawn({
              x: renderer.cursor.x - ax * 18 + px * spread * side,
              y: renderer.cursor.y - ay * 18 + py * spread * side,
              vx: -renderer.velocity.x * 0.15 + (Math.random() - 0.5) * 0.5,
              vy: -renderer.velocity.y * 0.15 + (Math.random() - 0.5) * 0.5,
          ...
          const bank = Math.max(0.5, 1 - Math.abs(renderer.velocity.x) * 0.003);
          ctx.scale(1, bank);
          ...
          if (renderer.speed > 5) {
            const flameLen = Math.min(renderer.speed * 0.6, 50);
            const flicker = Math.random() * 5;
            ctx.beginPath();
            ctx.moveTo(-10, -5);
            ctx.lineTo(-10 - flameLen - flicker, -6);
            ctx.lineTo(-10, -7);
            ctx.fill();
            ctx.beginPath();
            ctx.moveTo(-10, 5);
